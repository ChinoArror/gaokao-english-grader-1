...
const buildOverallHtml = (report: StructuredReport) => {
  const extra =
...
const buildLineCorrectionsHtml = (report: StructuredReport) => `
  ${sectionHeader('逐句批改')}
  <div class="paper-card">
    ${report.lineByLineCorrections
      .map(
        (item) => `
...
const buildErrorAnalysisHtml = (report: StructuredReport) => `
...
const buildPracticalHtml = (report: Extract<StructuredReport, { type: 'practical' }>) => `
  ...
  <div class="paper-card">
    ...
    <div class="sentence-block">
      ...
      ${report.personalizedImprovement.topicExpansion.sentences
        .map(
          (item) => `
...
const buildContinuationHtml = (report: Extract<StructuredReport, { type: 'continuation' }>) => `
  ${sectionHeader('情节分析')}
  <div class="paper-card">
...
export const buildPrintableReportHtmlV2 = (params: {
  ...
  const body = report.type === 'practical' ? buildPracticalHtml(report) : buildContinuationHtml(report);
  ...
  <head>
    ...
    <style>
      ...
      .page { width: 100%; max-width: 178mm; margin: 0 auto; padding-bottom: 20mm; }
      .meta { display: flex; justify-content: flex-end; font-size: 13px; margin-bottom: 6mm; }
      .title-main { text-align: center; font-size: 25px; font-weight: 800; line-height: 1.3; margin: 4mm 0 8mm; }
      .section-header { display: flex; align-items: center; gap: 8px; font-size: 17px; font-weight: 700; margin: 7mm 0 3mm; }
      .section-header .line { flex: 1; border-top: 1px dashed #64748b; }
      .paper-card { font-size: 14px; line-height: 1.8; }
      .paper-card p { margin: 1.5mm 0; }
      .sentence-block { margin: 4mm 0; break-inside: avoid; }
      .label { font-weight: 700; margin-right: 4px; }
      .handwriting-row { display: flex; align-items: center; gap: 10px; margin: 4mm 0; }
      .pill-label, .pill-value { display: inline-flex; align-items: center; justify-content: center; min-width: 105px; height: 34px; border-radius: 999px; font-size: 13px; font-weight: 700; }
      .pill-label { background: #1f2937; color: white; }
      .pill-value { background: #fae6d8; color: #111827; }
      .report-table { width: 100%; border-collapse: collapse; margin: 2mm 0 4mm; font-size: 13px; }
      .report-table th, .report-table td { border: 1px solid #475569; padding: 8px 10px; vertical-align: top; }
      ...
      h3 { margin: 4mm 0 2mm; font-size: 16px; }
      .essay-title { text-align: center; font-size: 21px; font-weight: 800; margin-bottom: 4mm; }
      .essay-content { white-space: pre-wrap; line-height: 1.9; font-size: 15px; text-align: left; }
      .footer { position: fixed; left: 16mm; right: 16mm; bottom: 8mm; display: flex; justify-content: space-between; font-size: 12px; color: #111827; }
    </style>
  </head>
  <body>
    <div class="page">
      <div class="meta">${escapeHtml(report.type === 'practical' ? '应用文批改' : '读后续写批改')}</div>
      <div class="title-main">${escapeHtml(title)}</div>
      ${buildOverallHtml(report)}
      ${buildLineCorrectionsHtml(report)}
      ${buildErrorAnalysisHtml(report)}
      ${body}
      ${sectionHeader('作文润色')}
      <div class="paper-card">
        <div class="essay-title">${escapeHtml(title)}</div>
...
export const openPrintableReportV2 = (params: {
  report: StructuredReport;
  topic?: string;
  originalContent?: string;
  dateText?: string;
  ...
  printWindow.document.write(buildPrintableReportHtmlV2(params));
